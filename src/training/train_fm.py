...
def get_batch(FM : FlowMatcher, x0 : th.Tensor, x1 : th.Tensor, return_noise=False):
...
def restart_func(restart_epoch, path, model, optimizer, sched=None):
    ...
    state_dict = th.load(f'{path}/checkpoint_{restart_epoch}.pth', weights_only=True)
...
def train_model(model: nn.Module, FM: FlowMatcher, train_dataloader,
                ...
                device, return_noise=False, class_cond=False,
                restart=False, restart_epoch=None, is_base_gaussian=False):

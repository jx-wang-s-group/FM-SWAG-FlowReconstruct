...
class SWAG(nn.Module):
    def __init__(self, base_model, device, variance_clamp=1e-6, max_rank=20):
        ...
        self.register_buffer('mean', th.zeros(self.num_parameters, device=self.model_device))  
        self.register_buffer('square_mean', th.zeros(self.num_parameters, device=self.model_device))  
        self.register_buffer('num_collected_models', th.zeros(1, dtype=th.long, device=self.model_device))
        self.register_buffer('rank', th.zeros(1, dtype=th.long, device=self.model_device))
        self.register_buffer('cov_mat_sqrt', th.empty(self.max_rank, self.num_parameters, dtype=th.float32, device=self.model_device))
    ...
    def collect_model(self, model_instance):
        flat_params = th.cat([p.detach().cpu().view(-1) for p in model_instance.parameters()]).to(self.model_device)
        ...
        self.cov_mat_sqrt = th.cat((self.cov_mat_sqrt, (flat_params - self.mean).unsqueeze(0)), dim=0)
        self.rank = th.clamp(self.rank + 1, max=self.max_rank)
        self.num_collected_models += 1

    def _get_mean_and_variance(self):
        variance = th.clamp(self.square_mean - self.mean.pow(2), self.variance_clamp)
    ...
    def sample(self, scale=0.5, diagonal_noise=True):
        ...
        low_rank_noise = th.randn(self.covariance_factor.size(0), device=self.model_device)
        scaled_noise = self.covariance_factor.t().matmul(low_rank_noise)

        if diagonal_noise:
            scaled_noise += variance.sqrt() * th.randn_like(variance)
